...
# 1. 生成 metadata.jsonl
echo "[1/3] 生成 metadata.jsonl"       | tee -a "$LOG"
python3 data_prep.py 2>&1             | tee -a "$LOG"

# 2. 校验 metadata.jsonl
echo "[2/3] 校验 metadata.jsonl"       | tee -a "$LOG"
